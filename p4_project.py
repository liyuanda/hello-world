...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  5 14:39:39 2018
@author: Yuanda
"""
...
def gconnect():
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('Sser is already connected.'),
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def categoriesJSON():
    ...
    for c in range(len(categories_dict)):
        items = [i.serialize for i in session.query(Item).
                 filter_by(categories_id=categories_dict[c]["id"]).all()]
        if items:
            categories_dict[c]["Item"] = items
...
def allCategories(categories_id):
    categories = session.query(Categories).filter_by(id=categories_id).one()
    items = session.query(Item).filter_by(categories_id=categories.id).all()
...
@app.route('/categories/<int:item_id>/description/')
def itemDescription(item_id):
    items = session.query(Item).filter_by(id=item_id).one()
    return render_template('itemDescrip.html', items=items)


@app.route('/categories/<int:categories_id>/new/', methods=['GET', 'POST'])
def newCatItem(categories_id):
    if 'username' not in login_session:
        return redirect('/login')
    if request.method == 'POST':
        newItem = Item(name=request.form['name'], categories_id=categories_id,
                       description=request.form['description'],
                       user_id=login_session['user_id'])
        session.add(newItem)
        session.commit()
        flash("New item added!")
        return redirect(url_for('allCategories', categories_id=categories_id))
    else:
        return render_template('newCatItem.html', categories_id=categories_id)
...
@app.route('/categories/<int:categories_id>/<int:item_id>/edit/',
           methods=['GET', 'POST'])
def editCatItem(categories_id, item_id):
    if 'username' not in login_session:
        return redirect('login')
    ...
    if request.method == 'POST':
        ...
        session.commit(), flash("Item edited")
...
@app.route('/categories/<int:categories_id>/<int:item_id>/delete/',
           methods=['GET', 'POST'])
def deleteCatItem(categories_id, item_id):
    if 'username' not in login_session:
        return redirect('login')
